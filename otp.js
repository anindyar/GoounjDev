        ...
        if((request.body.authCode != null) && (request.body.userId != null)) {
            request.getConnection(function(connectionError, connection) {
                if(connectionError != null) {
                    log.error(connectionError, "Database connection error (Function = verifyotp)");
                    json = {
                ...
                connection.query('SELECT auth_code FROM '+ config.mysql.db.name +'.two_step_verification WHERE user_id = ?', request.body.userId, function(queryError, match) {
                    ...
                    if(match[0]) {
                        if(match[0].auth_code == request.body.authCode) {
                            log.info({Function: "verifyotp"}, "otp verification for vote successful.");
                            return response.sendStatus(200);
                        }
                        else {
                            log.info({Function: "verifyotp"}, "otp verification for vote unsuccessful.");
...
exports.sendotp = function(request, response) {
    ...
    var authCode = Math.floor(Math.random() * 9000) + 1000;
        ...
        request.getConnection(function(connectionError, connection) {
            if(connectionError != null) {
                log.error(connectionError, "Database connection error (Function = sendotp)");
                json = {
                    error: "Requested action failed. Database could not be reached."
                };
                return response.status(500).json(json);
            ...
            connection.query('SELECT phone FROM '+ config.mysql.db.name +'.user WHERE id = ?', request.params.id, function(queryError, number) {
                if(queryError != null) {
                    log.error(queryError, "Query error. Failed to generate otp. (Function = sendotp)");
                    json = {
                        error: "Requested action failed. Database could not be reached."
                    };
                    return response.status(500).json(json);
                ...
                if(number[0]) {
                        ...
                        if(check.length != 0) {
                            connection.query('UPDATE '+ config.mysql.db.name +'.two_step_verification SET auth_code = ? WHERE user_id = ?', [authCode, request.params.id], function(queryError, fix) {
                                if(queryError != null) {
                                    ...
                                    log.info({Function: "sendotp"}, "otp generation successful.");
                                    return response.sendStatus(200);
                                }
                            });
                        }
                            ...
                            connection.query('INSERT INTO '+ config.mysql.db.name +'.two_step_verification (user_id, auth_code) VALUES (?, ?)',[request.params.id, authCode], function(queryError, entry) {
                                if(queryError != null) {
                                    ...
                                    log.info({Function: "sendotp"}, "otp generation for vote successful.");
                                    return response.sendStatus(200);
                                }
                            });
                        }
                    ...
                    log.info({Function: "sendotp"}, "User not found.");
