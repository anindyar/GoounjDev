    ...
    try {
        if((request.body.userId !== null) && (request.body.limit != null)) {
            request.getConnection(function (connectionError, connection) {
                ...
                connection.query('(SELECT poll.id AS pollId, start_date AS startDate, end_date AS endDate, poll_name AS pollName, is_boost AS isBoost, poll.is_active AS isActive, is_generic AS isGeneric, (SELECT concat(first_name," ",last_name)) AS createdUserName  FROM poll INNER JOIN user ON poll.created_user_id = user.id INNER JOIN audience_poll_map ON poll.id = poll_id WHERE poll.end_date > ? AND user_id = ?) UNION (SELECT poll.id AS pollId, start_date AS startDate, end_date AS endDate, poll_name AS pollName, is_boost AS isBoost, poll.is_active AS isActive, is_generic AS isGeneric, (SELECT concat(first_name," ",last_name)) AS createdUserName  FROM poll INNER JOIN user ON poll.created_user_id = user.id WHERE is_generic = ?) LIMIT ?', [utcTimeStamp, request.body.userId, "1", request.body.limit], function(queryError, result) {
                    if (queryError != null) {
