...
/**
 * @api {post} /survey/v1/surveyList Show polls for Audience
 * @apiVersion 0.1.0
    ...
    try {
        if((request.body.lowerLimit != null) && (request.body.upperLimit != null)) {
            request.getConnection(function (connectionError, connection) {
                ...
                connection.query('SELECT poll.id AS pollId, start_date AS startDate, end_date AS endDate, poll_name AS pollName, is_survey AS isSurvey, is_boost AS isBoost, poll.is_active AS isActive, is_generic AS isGeneric,  name AS createdUserName  FROM poll INNER JOIN user ON poll.created_user_id = user.id WHERE poll.end_date > ? AND is_survey = 1 ORDER BY pollId DESC LIMIT ?, ?', [utcTimeStamp, request.body.lowerLimit, request.body.upperLimit], function(queryError, result) {
                    if (queryError != null) {
...
/**
 * @api {get} /survey/v1/surveyList/:id Show survey polls for Created User
 * @apiVersion 0.1.0
